...
let
  cfg = vm.ownloomData or { };
  stateDir = cfg.stateDir or "/var/lib/ownloom-data";
  ...
  authRealm = auth.realm or "OwnLoom Data";
  htpasswdFile = auth.htpasswdFile or "/var/lib/ownloom-data/secrets/ownloom-data-htpasswd";
  ...
  gitBackupSshKey = gitBackup.sshKeyFile or "${stateDir}/secrets/wiki-backup-ed25519";
  gitBackupKnownHosts = gitBackup.knownHostsFile or "${stateDir}/secrets/wiki-backup-known_hosts";
...
in
{
  assertions = [
    {
      assertion = vm ? service && vm.service == "ownloom-data";
      message = "ownloom-data service module may only be enabled for fleet VMs with service = \"ownloom-data\".";
    }
  ];
  ...
  services.nginx = {
    ...
    virtualHosts.${vm.dns} = {
      ...
      locations."/".return = "200 'OwnLoom data VM: private DAV endpoints are /radicale/ and /files/.\n'";
  ...
  systemd.services.ownloom-wiki-git-backup = lib.mkIf gitBackupEnable {
    description = "Snapshot OwnLoom personal WebDAV wiki to git";
    ...
    script = ''
      ...
      git config user.name "OwnLoom Data Wiki Backup"
      git config user.email "ownloom-data@nazar.studio"
      ...
      if ! find ${gitBackupWorkTree} -mindepth 1 -maxdepth 1 ! -name .git | grep -q .; then
        printf 'OwnLoom personal wiki backup placeholder. Remove once the WebDAV wiki contains files.\n' > .backup-placeholder
      else
  ...
  systemd.timers.ownloom-wiki-git-backup = lib.mkIf gitBackupEnable {
    description = "Periodically snapshot OwnLoom personal WebDAV wiki to git";
    wantedBy = [ "timers.target" ];
    timerConfig = {
      OnCalendar = gitBackupCalendar;
      Persistent = true;
      Unit = "ownloom-wiki-git-backup.service";
    };
  };

  systemd.services.ownloom-data-auth-gate = {
    description = "Document OwnLoom data VM bootstrap auth gate";
    ...
    script = ''
      cat <<'EOF'
      OwnLoom data VM mode: Radicale and WebDAV are reachable through the NetBird interface only.
