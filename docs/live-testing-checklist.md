...
Audience: operators validating a fresh Workspace image on real hardware or a realistic VM.

## 🌱 Why This Checklist Exists

This is the acceptance checklist for first real Workspace runs.
...
2. Confirm `workspace-matrix.service` is active.
...
2. Corrupt `~/Workspace/guardrails.yaml` and confirm Workspace falls back to the packaged defaults instead of crashing the session startup path.
