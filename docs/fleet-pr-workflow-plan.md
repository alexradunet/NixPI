...
- [x] Add `bloom_repo(action="configure")` tool:
  - clone repo if missing
  - configure `upstream`
  - configure `origin` (fork URL or gh-assisted)
  - set repo-local git identity
- [x] Add `bloom_repo(action="status")` PR readiness view.
- [x] Add `bloom_repo(action="sync")` fast-forward sync helper.
...
2. Run `bloom_repo(action="configure")` once.
3. Run `bloom_repo(action="status")` and see PR-ready state.
...
- **Merge conflicts/drift** → `bloom_repo(action="sync")` before branching.
